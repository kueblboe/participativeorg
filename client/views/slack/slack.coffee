...
Template.slack.helpers
  ...
  hasGoals: -> Goals.find({date: { $gte: startOfYear(this.year), $lte: endOfYear(this.year) } }).count() > 0
  goals: -> Goals.find({date: { $gte: startOfYear(this.year), $lte: endOfYear(this.year) } })
